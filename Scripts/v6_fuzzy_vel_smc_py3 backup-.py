...
class fcuModes:
    def setTakeoff(self):
        ...
        try:
            ...
            print("Service takeoff call failed: " + str(e))
    ...
    def setArm(self, _to_arm):
        ...
        try:
            ...
            print("Service arming call failed: " + str(e))
    ...
    def setMode(self, _mode):
        if _mode in ("STABILIZED",
            ...
            try:
                ...
                print("service set_mode call failed: "+ str(e) + ". Offboard Mode could not be set.")
...
def test_run(in_X, in_Y, in_Z, dist):
    rospy.loginfo("++++++++ Begin test run")
    rospy.loginfo("++++++++ Origin: in_X -> " + str(in_X) + ", in_Y -> "+ str(in_Y) + ", in_Z -> "+ str(in_Z))

    _bgcs = 2
    rospy.loginfo("==== Move " + str(_bgcs) + "meters")
    ...
    rospy.sleep(2)
    rospy.loginfo("==== Make circle of radius " + str(_bgcs))
    ...
    _smcs = 1
    rospy.loginfo("==== Make little circles of radius " + str(_smcs))
    for angle in range(360):
    ...
    for direction in range(3):
        ...
        d_a[direction] = 1 
        for _ in range(2):
            rospy.loginfo("==== Move {} meters on dir {}".format(_altd, direction))
            ...
            setpoint_vel.set(in_X + d_a[0] * _altd,
                            ...
                            in_Z + d_a[2] * _altd,
                            wait=True, reaching_distance=dist)
            rospy.loginfo("==== Wait for {} seconds".format(_wttm))
            rospy.sleep(_wttm)
            rospy.loginfo("==== Return to origin")
            for position in range(int(_altd * 10000)):
...
def test_run_goto(trgt_X, trgt_Y, trgt_Z, alt, dist):
    ...
    orig_Y = DronePose.y
    orig_Z = DronePose.z
    rospy.loginfo("++++++++ Origin: orig_X -> {}, orig_Y -> {}, orig_Z -> {}".format(orig_X, orig_Y, orig_Z))
    rospy.loginfo("++++++++ Target: trgt_X -> {}, trgt_Y -> {}, trgt_Z -> {}".format(trgt_X, trgt_Y, trgt_Z))

    rospy.loginfo("==== Go up {} meters".format(alt))
