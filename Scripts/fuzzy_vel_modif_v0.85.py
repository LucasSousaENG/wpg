...
# Le a ultima distancia atualizada no arquivo txt
def read_distance():
    global depth_at_point
    # Try caso ocorra erro lendo um arquivo ao mesmo tempo que e modificado
    #try:
    with open('/home/lucas/Downloads/distance.txt', 'r') as arquivo:
        depth_at_point = arquivo.read().strip()
        #print("teste - " + depth_at_point)
...
def callback(msg):
    ...
    # Obtem novos dados de distancia
    read_distance()
    print("Nome do Parametro: distancia, Valor: " + str(depth_at_point))
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        pid_msg.kI = (ki)
        pid_msg.kD = (kd)
        pid_msg.obj_vel = float("{:.3f}".format(float(depth_at_point)))
